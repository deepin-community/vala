                                            ...
                                            ValaDataType* vsymbol)
{
		...
		first = TRUE;
		{
			...
			while (TRUE) {
				...
				if (!_tmp22_) {
					...
					valadoc_api_signature_builder_append_keyword (_tmp23_, "weak", TRUE);
				}
				_tmp24_ = self->priv->signature;
				_tmp25_ = type_arg;
				_tmp26_ = vala_data_type_to_qualified_string (_tmp25_, NULL);
				_tmp27_ = _tmp26_;
				valadoc_api_signature_builder_append (_tmp24_, _tmp27_, TRUE);
				_g_free0 (_tmp27_);
	...
	_tmp29_ = vala_data_type_get_nullable (vsymbol);
	_tmp30_ = _tmp29_;
	if (_tmp30_) {
                                                                      ...
                                                                      ValaArrayCreationExpression* expr)
{
	...
	first = TRUE;
	{
		...
		while (TRUE) {
			...
			_tmp10_ = _size_list;
			_tmp11_ = vala_list_get (_tmp10_, _size_index);
	...
	_tmp15_ = vala_array_creation_expression_get_initializer_list (expr);
	_tmp16_ = _tmp15_;
	if (_tmp16_ != NULL) {
                                                        ...
                                                        ValaMethodCall* expr)
{
	...
	first = TRUE;
	{
		...
		_tmp5_ = vala_callable_expression_get_argument_list ((ValaCallableExpression*) expr);
		...
		while (TRUE) {
			...
			_tmp10_ = _literal_size;
			if (!(_tmp9_ < _tmp10_)) {
				break;
			}
			_tmp11_ = _literal_list;
			_tmp12_ = vala_list_get (_tmp11_, _literal_index);
			literal = (ValaExpression*) _tmp12_;
			if (!first) {
				ValadocApiSignatureBuilder* _tmp13_;
				_tmp13_ = self->priv->signature;
				valadoc_api_signature_builder_append (_tmp13_, ", ", FALSE);
                                                                       ...
                                                                       ValaObjectCreationExpression* expr)
{
	...
	first = TRUE;
	{
		...
		gint _tmp12_;
		gint _arg_index = 0;
		_tmp9_ = vala_callable_expression_get_argument_list ((ValaCallableExpression*) expr);
		_arg_list = _tmp9_;
		_tmp10_ = _arg_list;
                                                              ...
                                                              ValaLambdaExpression* expr)
{
	...
	first = TRUE;
	{
		...
		while (TRUE) {
			...
			_tmp13_ = _tmp12_;
			valadoc_api_signature_builder_append (_tmp10_, _tmp13_, FALSE);
			first = FALSE;
	...
	valadoc_api_signature_builder_append (_tmp14_, ") => {", FALSE);
	_tmp15_ = self->priv->signature;
	valadoc_api_signature_builder_append_highlighted (_tmp15_, " [...] ", FALSE);
