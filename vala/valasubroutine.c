...
{
	ValaSubroutineClass* _klass_;
	g_return_val_if_fail (self != NULL, FALSE);
	_klass_ = VALA_SUBROUTINE_GET_CLASS (self);
	if (_klass_->get_has_result) {
		return _klass_->get_has_result (self);
	}
	return FALSE;
}
...
{
	static volatile gsize vala_subroutine_type_id__once = 0;
	if (g_once_init_enter (&vala_subroutine_type_id__once)) {
		GType vala_subroutine_type_id;
		vala_subroutine_type_id = vala_subroutine_get_type_once ();
		g_once_init_leave (&vala_subroutine_type_id__once, vala_subroutine_type_id);
	}
	return vala_subroutine_type_id__once;
}
