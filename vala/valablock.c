...
gboolean
vala_block_get_unreachable_exit (ValaBlock* self)
{
	gboolean result;
	g_return_val_if_fail (self != NULL, FALSE);
	result = self->priv->_unreachable_exit;
	return result;
}

void
vala_block_set_unreachable_exit (ValaBlock* self,
                                 gboolean value)
{
	g_return_if_fail (self != NULL);
	self->priv->_unreachable_exit = value;
}
...
vala_block_get_statements (ValaBlock* self)
{
	...
	list = _tmp1_;
	{
		...
		while (TRUE) {
			...
			_tmp8_ = _stmt_list;
			_tmp9_ = vala_list_get (_tmp8_, _stmt_index);
			stmt = (ValaStatement*) _tmp9_;
			_tmp10_ = stmt;
			stmt_list = VALA_IS_STATEMENT_LIST (_tmp10_) ? ((ValaStatementList*) _tmp10_) : NULL;
			_tmp11_ = stmt_list;
			if (_tmp11_ != NULL) {
				{
					gint i = 0;
                               ...
                               ValaLocalVariable* local)
{
	...
	while (TRUE) {
		...
		if (_tmp14_) {
			...
			_tmp18_ = _tmp17_;
			vala_report_error (_tmp16_, "Local variable `%s' conflicts with a local variable or constant declar" \
"ed in a parent scope", _tmp18_);
			break;
		}
		_tmp19_ = parent_block;
		_tmp20_ = vala_symbol_get_parent_symbol (_tmp19_);
		_tmp21_ = _tmp20_;
		parent_block = _tmp21_;
	}
	_tmp22_ = self->priv->local_variables;
	vala_collection_add ((ValaCollection*) _tmp22_, local);
}
                               ...
                               ValaConstant* constant)
{
	...
	while (TRUE) {
		...
		if (_tmp14_) {
			...
			_tmp18_ = _tmp17_;
			vala_report_error (_tmp16_, "Local constant `%s' conflicts with a local variable or constant declar" \
"ed in a parent scope", _tmp18_);
			break;
		}
		_tmp19_ = parent_block;
	...
	_tmp22_ = self->priv->local_constants;
	vala_collection_add ((ValaCollection*) _tmp22_, constant);
	_tmp23_ = vala_symbol_get_scope ((ValaSymbol*) self);
	_tmp24_ = _tmp23_;
	_tmp25_ = vala_symbol_get_name ((ValaSymbol*) constant);
	_tmp26_ = _tmp25_;
	vala_scope_add (_tmp24_, _tmp26_, (ValaSymbol*) constant);
}
                       ...
                       ValaCodeContext* context)
{
	...
	}
	{
		...
		while (TRUE) {
			...
			_tmp39_ = _local_list;
			_tmp40_ = vala_list_get (_tmp39_, _local_index);
			local = (ValaLocalVariable*) _tmp40_;
			_tmp41_ = local;
			vala_symbol_set_active ((ValaSymbol*) _tmp41_, FALSE);
	...
	}
	{
		...
		_tmp42_ = self->priv->local_constants;
		_constant_list = _tmp42_;
		_tmp43_ = _constant_list;
		_tmp44_ = vala_collection_get_size ((ValaCollection*) _tmp43_);
		_tmp45_ = _tmp44_;
		_constant_size = _tmp45_;
		_constant_index = -1;
		while (TRUE) {
			gint _tmp46_;
			gint _tmp47_;
			...
			_tmp46_ = _constant_index;
			_tmp47_ = _constant_size;
			if (!(_tmp46_ < _tmp47_)) {
				break;
			}
			_tmp48_ = _constant_list;
			_tmp49_ = vala_list_get (_tmp48_, _constant_index);
			constant = (ValaConstant*) _tmp49_;
			_tmp50_ = constant;
	...
	_tmp52_ = _tmp51_;
	_tmp53_ = old_symbol;
	vala_semantic_analyzer_set_current_symbol (_tmp52_, _tmp53_);
	_tmp54_ = vala_code_context_get_analyzer (context);
	_tmp55_ = _tmp54_;
	_tmp56_ = old_insert_block;
	_tmp57_ = _vala_code_node_ref0 (_tmp56_);
	_vala_code_node_unref0 (_tmp55_->insert_block);
	_tmp55_->insert_block = _tmp57_;
