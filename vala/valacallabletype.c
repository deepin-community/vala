                                             ...
                                             const gchar* override_name)
{
	...
	ValaCallable* _tmp111_;
	ValaCallable* _tmp112_;
	...
	}
	{
		...
		while (TRUE) {
			...
			_tmp60_ = param;
			_tmp61_ = vala_parameter_get_params_array (_tmp60_);
			_tmp62_ = _tmp61_;
			if (_tmp62_) {
				GString* _tmp63_;
				_tmp63_ = builder;
				g_string_append (_tmp63_, "params ");
			}
			_tmp64_ = param;
			...
			if (_tmp66_ == VALA_PARAMETER_DIRECTION_IN) {
				...
				_tmp69_ = _tmp68_;
				_tmp70_ = vala_data_type_get_value_owned (_tmp69_);
				_tmp71_ = _tmp70_;
				if (_tmp71_) {
					GString* _tmp72_;
					_tmp72_ = builder;
					g_string_append (_tmp72_, "owned ");
				}
			} else {
				...
				if (_tmp75_ == VALA_PARAMETER_DIRECTION_REF) {
					...
					_tmp79_ = _tmp78_;
					if (_tmp79_ == VALA_PARAMETER_DIRECTION_OUT) {
						GString* _tmp80_;
						_tmp80_ = builder;
						g_string_append (_tmp80_, "out ");
					}
				}
				_tmp82_ = param;
				...
				if (!_tmp86_) {
					...
					_tmp89_ = _tmp88_;
					_tmp81_ = VALA_IS_REFERENCE_TYPE (_tmp89_);
				} else {
					_tmp81_ = FALSE;
				}
				if (_tmp81_) {
			...
			_tmp94_ = _tmp93_;
			_tmp95_ = vala_data_type_to_qualified_string (_tmp94_, NULL);
			_tmp96_ = _tmp95_;
			g_string_append (_tmp91_, _tmp96_);
			_g_free0 (_tmp96_);
			_tmp97_ = param;
			_tmp98_ = vala_variable_get_initializer ((ValaVariable*) _tmp97_);
			_tmp99_ = _tmp98_;
			if (_tmp99_ != NULL) {
				GString* _tmp100_;
				GString* _tmp101_;
	...
	_tmp110_ = vala_array_list_new (VALA_TYPE_DATA_TYPE, (GBoxedCopyFunc) vala_code_node_ref, (GDestroyNotify) vala_code_node_unref, _tmp109_);
	error_types = _tmp110_;
	_tmp111_ = vala_callable_type_get_callable_symbol (self);
	_tmp112_ = _tmp111_;
	_tmp113_ = error_types;
	vala_code_node_get_error_types ((ValaCodeNode*) _tmp112_, (ValaCollection*) _tmp113_, NULL);
	...
	if (_tmp116_ > 0) {
		...
		_tmp117_ = builder;
		g_string_append (_tmp117_, " throws ");
		first = TRUE;
		{
