                                   ...
                                   ValaSourceFile* file)
{
	...
	if (vala_map_contains (_tmp0_, _tmp2_)) {
		...
		_tmp4_ = _tmp3_;
		vala_report_warning (NULL, "Ignoring source file `%s', which was already added to this context", _tmp4_);
		return;
	}
	_tmp5_ = self->priv->source_files;
	vala_collection_add ((ValaCollection*) _tmp5_, file);
	_tmp6_ = self->priv->source_files_map;
	_tmp7_ = vala_source_file_get_filename (file);
	_tmp8_ = _tmp7_;
	vala_map_set (_tmp6_, _tmp8_, file);
}
...
"trospection GIR directories", pkg);
	...
	if (g_strcmp0 (_tmp9_, _tmp10_) != 0) {
		...
		_tmp11_ = self->priv->source_files_map;
		_tmp12_ = rpath;
		_tmp13_ = source_file;
		vala_map_set (_tmp11_, _tmp12_, _tmp13_);
	}
                                          ...
                                          const gchar* filename)
{
	...
	__catch0_g_file_error:
	{
		...
		vala_report_error (NULL, "Unable to read dependency file: %s", _tmp11_);
		result = FALSE;
		_g_error_free0 (e);
		return result;
	}
                                       ...
                                       gboolean cmdline)
{
	...
	g_return_val_if_fail (filename != NULL, FALSE);
	if (!g_file_test (filename, G_FILE_TEST_EXISTS)) {
		vala_report_error (NULL, "%s not found", filename);
		result = FALSE;
		return result;
	}
	_tmp0_ = vala_code_context_realpath (filename);
	rpath = _tmp0_;
	if (is_source) {
		_tmp2_ = TRUE;
	} else {
		_tmp2_ = g_str_has_suffix (filename, ".vala");
	}
	if (_tmp2_) {
		_tmp1_ = TRUE;
	} else {
		_tmp1_ = g_str_has_suffix (filename, ".gs");
	}
	if (_tmp1_) {
		ValaSourceFile* source_file = NULL;
		const gchar* _tmp3_;
		...
		const gchar* _tmp27_;
		_tmp3_ = rpath;
		_tmp4_ = vala_source_file_new (self, VALA_SOURCE_FILE_TYPE_SOURCE, _tmp3_, NULL, cmdline);
		source_file = _tmp4_;
		_tmp5_ = source_file;
		vala_source_file_set_relative_filename (_tmp5_, filename);
		_tmp6_ = self->priv->_profile;
		if (_tmp6_ == VALA_PROFILE_POSIX) {
			...
			_tmp7_ = vala_unresolved_symbol_new (NULL, "Posix", NULL);
			_tmp8_ = _tmp7_;
			_tmp9_ = vala_using_directive_new ((ValaSymbol*) _tmp8_, NULL);
			_tmp10_ = _tmp9_;
			...
			_tmp14_ = _tmp13_;
			_tmp15_ = ns_ref;
			vala_namespace_add_using_directive (_tmp14_, _tmp15_);
			_vala_code_node_unref0 (ns_ref);
		} else {
			ValaProfile _tmp16_;
			_tmp16_ = self->priv->_profile;
			if (_tmp16_ == VALA_PROFILE_GOBJECT) {
				ValaUsingDirective* ns_ref = NULL;
				ValaUnresolvedSymbol* _tmp17_;
				ValaUnresolvedSymbol* _tmp18_;
				ValaUsingDirective* _tmp19_;
				ValaUsingDirective* _tmp20_;
				ValaSourceFile* _tmp21_;
				ValaUsingDirective* _tmp22_;
				ValaNamespace* _tmp23_;
				ValaNamespace* _tmp24_;
				ValaUsingDirective* _tmp25_;
				_tmp17_ = vala_unresolved_symbol_new (NULL, "GLib", NULL);
				_tmp18_ = _tmp17_;
				_tmp19_ = vala_using_directive_new ((ValaSymbol*) _tmp18_, NULL);
				_tmp20_ = _tmp19_;
				_vala_code_node_unref0 (_tmp18_);
				ns_ref = _tmp20_;
				_tmp21_ = source_file;
				_tmp22_ = ns_ref;
				vala_source_file_add_using_directive (_tmp21_, _tmp22_);
				_tmp23_ = vala_code_context_get_root (self);
				_tmp24_ = _tmp23_;
				_tmp25_ = ns_ref;
				vala_namespace_add_using_directive (_tmp24_, _tmp25_);
				_vala_code_node_unref0 (ns_ref);
			}
		}
		_tmp26_ = source_file;
		vala_code_context_add_source_file (self, _tmp26_);
		_tmp27_ = rpath;
		if (g_strcmp0 (_tmp27_, filename) != 0) {
			ValaMap* _tmp28_;
			ValaSourceFile* _tmp29_;
			_tmp28_ = self->priv->source_files_map;
			_tmp29_ = source_file;
			vala_map_set (_tmp28_, filename, _tmp29_);
		}
		_vala_source_file_unref0 (source_file);
	} else {
		...
		if (g_str_has_suffix (filename, ".vapi")) {
			...
			_tmp30_ = g_str_has_suffix (filename, ".gir");
		}
		if (_tmp30_) {
			ValaSourceFile* source_file = NULL;
			const gchar* _tmp31_;
			...
			const gchar* _tmp35_;
			_tmp31_ = rpath;
			_tmp32_ = vala_source_file_new (self, VALA_SOURCE_FILE_TYPE_PACKAGE, _tmp31_, NULL, cmdline);
			source_file = _tmp32_;
			_tmp33_ = source_file;
			vala_source_file_set_relative_filename (_tmp33_, filename);
			_tmp34_ = source_file;
			vala_code_context_add_source_file (self, _tmp34_);
			_tmp35_ = rpath;
			if (g_strcmp0 (_tmp35_, filename) != 0) {
				ValaMap* _tmp36_;
				ValaSourceFile* _tmp37_;
				_tmp36_ = self->priv->source_files_map;
				_tmp37_ = source_file;
				vala_map_set (_tmp36_, filename, _tmp37_);
			}
			_vala_source_file_unref0 (source_file);
		} else {
			if (g_str_has_suffix (filename, ".c")) {
				const gchar* _tmp38_;
				_tmp38_ = rpath;
				vala_code_context_add_c_source_file (self, _tmp38_);
			} else {
                              ...
                              const gchar* define)
{
	...
	if (vala_code_context_is_defined (self, define)) {
		vala_report_warning (NULL, "`%s' is already defined", define);
                                      ...
                                      gboolean include_stdpkg)
{
	...
	switch (profile) {
		...
		{
			vala_code_context_set_profile (self, profile);
			vala_code_context_add_define (self, "LIBC");
                                      ...
                                      const gchar* filename)
{
	...
	fprintf (_tmp2_, "%s:", filename);
	{
		...
		while (TRUE) {
			...
			_tmp9_ = _src_list;
			_tmp10_ = vala_list_get (_tmp9_, _src_index);
			src = (ValaSourceFile*) _tmp10_;
			_tmp12_ = src;
			_tmp13_ = vala_source_file_get_file_type (_tmp12_);
			_tmp14_ = _tmp13_;
			if (_tmp14_ == VALA_SOURCE_FILE_TYPE_FAST) {
				ValaSourceFile* _tmp15_;
				gboolean _tmp16_;
				gboolean _tmp17_;
				_tmp15_ = src;
				_tmp16_ = vala_source_file_get_used (_tmp15_);
				_tmp17_ = _tmp16_;
				_tmp11_ = _tmp17_;
			} else {
				_tmp11_ = FALSE;
			...
			if (_tmp11_) {
				...
				_tmp18_ = stream;
				_tmp19_ = src;
				_tmp20_ = vala_source_file_get_filename (_tmp19_);
				_tmp21_ = _tmp20_;
                                               ...
                                               const gchar* filename)
{
	...
	first = TRUE;
	{
		...
		while (TRUE) {
			...
			_tmp8_ = _src_list;
			_tmp9_ = vala_list_get (_tmp8_, _src_index);
			src = (ValaSourceFile*) _tmp9_;
			_tmp11_ = src;
			_tmp12_ = vala_source_file_get_file_type (_tmp11_);
			_tmp13_ = _tmp12_;
			if (_tmp13_ != VALA_SOURCE_FILE_TYPE_SOURCE) {
				ValaSourceFile* _tmp14_;
				gboolean _tmp15_;
				gboolean _tmp16_;
				_tmp14_ = src;
				_tmp15_ = vala_source_file_get_used (_tmp14_);
				_tmp16_ = _tmp15_;
				_tmp10_ = _tmp16_;
			} else {
				_tmp10_ = FALSE;
                                            ...
                                            const gchar* package_name)
{
	...
	output = NULL;
	{
		...
		if (exit_status != 0) {
			...
			vala_report_error (NULL, "%s exited with status %d", _tmp10_, exit_status);
			result = NULL;
			_g_free0 (output);
			_g_free0 (pc);
			return result;
	...
	__catch0_g_spawn_error:
	{
		...
		vala_report_error (NULL, "%s", _tmp12_);
		_g_free0 (output);
		output = NULL;
		_g_error_free0 (e);
	}
