...
{
	ValadocImporterDocumentationImporterClass* _klass_;
	g_return_val_if_fail (self != NULL, NULL);
	_klass_ = VALADOC_IMPORTER_DOCUMENTATION_IMPORTER_GET_CLASS (self);
	if (_klass_->get_file_extension) {
		return _klass_->get_file_extension (self);
	}
	return NULL;
}
                                                 ...
                                                 const gchar* path)
{
	...
	_klass_ = VALADOC_IMPORTER_DOCUMENTATION_IMPORTER_GET_CLASS (self);
	if (_klass_->resolve) {
		return _klass_->resolve (self, path);
	}
	return NULL;
}
