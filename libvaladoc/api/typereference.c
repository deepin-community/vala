	...
	if (_tmp23_ > 0) {
		...
		first = TRUE;
		{
			...
			while (TRUE) {
				...
				_tmp31_ = _param_list;
				_tmp32_ = vala_list_get ((ValaList*) _tmp31_, _param_index);
				param = (ValadocApiItem*) ((ValadocApiTypeReference*) _tmp32_);
				if (!first) {
				...
				_tmp34_ = signature;
				_tmp35_ = param;
				_tmp36_ = valadoc_api_item_get_signature (_tmp35_);
				_tmp37_ = _tmp36_;
				valadoc_api_signature_builder_append_content (_tmp34_, _tmp37_, FALSE);
				first = FALSE;
				_g_object_unref0 (param);
			}
		}
		_tmp38_ = signature;
		valadoc_api_signature_builder_append (_tmp38_, ">", FALSE);
	}
	_tmp39_ = self->priv->_is_nullable;
	if (_tmp39_) {
